...
test('Handles string based arrays', t => {
  t.deepEqual(replaceAtIndex(1, 'haha', ['foo', 'bar', 'bax']), ['foo', 'haha', 'bax']);
});

test('It only replaces the index it was told', t => {
  t.deepEqual(replaceAtIndex(1, 10, [0, 1, 1, 2, 2, 3]), [0, 10, 1, 2, 2, 3]);
});

test('Is curried', t => {
  t.deepEqual(replaceAtIndex(2)(10)([0, 1, 2, 3]), [0, 1, 10, 3]);
});
